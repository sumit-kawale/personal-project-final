...
import { DecServeService } from '../month/dec/dec-serve.service';
...
export class DecemberComponent implements OnInit {
  ...
  constructor(
    public decServe: DecServeService,
  ...
  ngOnInit(): void {
    this.decServe.getBudget().subscribe(data => {
  ...
  addRecord(){
    ...
    this.decServe.addBudget(package1).then(res=>{
      this.BudgetTitle="";
  ...
  updateData(UserValue){
    ...
    this.decServe.editBudget(UserValue.id, payloadData);

    this.createchart();
  }

  deletedata(titleiddelete){
    this.decServe.deleteBudget(titleiddelete);
    setTimeout(() => {
      this.createchart();
     location.reload();
    }, 100);
