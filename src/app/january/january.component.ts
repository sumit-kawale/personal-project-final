...
import { JanServeService } from '../month/jan/jan-serve.service';
...
export class JanuaryComponent implements OnInit {
  ...
  constructor(
    public janServe: JanServeService,
  ...
  ngOnInit(): void {
    this.janServe.getBudget().subscribe(data => {
  ...
  addRecord(){
    ...
    this.janServe.addBudget(package1).then(res=>{
      this.BudgetTitle="";
  ...
  updateData(UserValue){
    ...
    this.janServe.editBudget(UserValue.id, payloadData);

    this.createchart();
  }

  deletedata(titleiddelete){
    this.janServe.deleteBudget(titleiddelete);
    setTimeout(() => {
      this.createchart();
     location.reload();
    }, 100);
