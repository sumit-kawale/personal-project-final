...
import { JulServeService } from '../month/jul/jul-serve.service';
...
export class JulyComponent implements OnInit {
  ...
  constructor(
    public julServe: JulServeService,
  ...
  ngOnInit(): void {
    this.julServe.getBudget().subscribe(data => {
  ...
  addRecord(){
    ...
    this.julServe.addBudget(package1).then(res=>{
      this.BudgetTitle="";
  ...
  updateData(UserValue){
    ...
    this.julServe.editBudget(UserValue.id, payloadData);

    this.createchart();
  }

  deletedata(titleiddelete){
    this.julServe.deleteBudget(titleiddelete);
    setTimeout(() => {
      this.createchart();
     location.reload();
    }, 100);
