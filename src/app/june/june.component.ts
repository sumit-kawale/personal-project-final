...
import { JunServeService } from '../month/jun/jun-serve.service';
...
export class JuneComponent implements OnInit {
  ...
  constructor(
    public junServe: JunServeService,
  ...
  ngOnInit(): void {
    this.junServe.getBudget().subscribe(data => {
  ...
  addRecord(){
    ...
    this.junServe.addBudget(package1).then(res=>{
      this.BudgetTitle="";
  ...
  updateData(UserValue){
    ...
    this.junServe.editBudget(UserValue.id, payloadData);

    this.createchart();
  }

  deletedata(titleiddelete){
    this.junServe.deleteBudget(titleiddelete);
    setTimeout(() => {
      this.createchart();
     location.reload();
    }, 100);
