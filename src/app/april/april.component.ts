...
import { AprServeService } from '../month/apr/apr-serve.service';
...
export class AprilComponent implements OnInit {
  ...
  constructor(
    public aprServe: AprServeService,
  ...
  ngOnInit(): void {
    this.aprServe.getBudget().subscribe(data => {
  ...
  addRecord(){
    ...
    this.aprServe.addBudget(package1).then(res=>{
      this.BudgetTitle="";
  ...
  updateData(UserValue){
    ...
    this.aprServe.editBudget(UserValue.id, payloadData);

    this.createchart();
  }

  deletedata(titleiddelete){
    this.aprServe.deleteBudget(titleiddelete);
    setTimeout(() => {
      this.createchart();
     location.reload();
    }, 100);
