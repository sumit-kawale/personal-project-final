...
import { MayServeService } from '../month/may/may-serve.service';
...
export class MayComponent implements OnInit {
  ...
  constructor(
    public mayServe: MayServeService,
  ...
  ngOnInit(): void {
    this.mayServe.getBudget().subscribe(data => {
  ...
  addRecord(){
    ...
    this.mayServe.addBudget(package1).then(res=>{
      this.BudgetTitle="";
  ...
  updateData(UserValue){
    ...
    this.mayServe.editBudget(UserValue.id, payloadData);

    this.createchart();
  }

  deletedata(titleiddelete){
    this.mayServe.deleteBudget(titleiddelete);
    setTimeout(() => {
      this.createchart();
     location.reload();
    }, 100);
