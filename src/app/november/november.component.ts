...
import { NovServeService } from '../month/nov/nov-serve.service';
...
export class NovemberComponent implements OnInit {
  ...
  constructor(
    public novServe: NovServeService,
  ...
  ngOnInit(): void {
    this.novServe.getBudget().subscribe(data => {
  ...
  addRecord(){
    ...
    this.novServe.addBudget(package1).then(res=>{
      this.BudgetTitle="";
  ...
  updateData(UserValue){
    ...
    this.novServe.editBudget(UserValue.id, payloadData);

    this.createchart();
  }

  deletedata(titleiddelete){
    this.novServe.deleteBudget(titleiddelete);
    setTimeout(() => {
      this.createchart();
     location.reload();
    }, 100);
