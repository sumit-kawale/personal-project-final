...
import { OctServeService } from '../month/oct/oct-serve.service';
...
export class OctoberComponent implements OnInit {
  ...
  constructor(
    public octServe: OctServeService,
  ...
  ngOnInit(): void {
    this.octServe.getBudget().subscribe(data => {
  ...
  addRecord(){
    ...
    this.octServe.addBudget(package1).then(res=>{
      this.BudgetTitle="";
  ...
  updateData(UserValue){
    ...
    this.octServe.editBudget(UserValue.id, payloadData);

    this.createchart();
  }

  deletedata(titleiddelete){
    this.octServe.deleteBudget(titleiddelete);
    setTimeout(() => {
      this.createchart();
     location.reload();
    }, 100);
